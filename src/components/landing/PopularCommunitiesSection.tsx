      ...
      <CardContent className="p-3">
        <div className="space-y-3">
          {communities.map((community) => (
            <div key={community.id} className="flex items-center p-2 hover:bg-premium-dark-700/40 rounded-md transition-colors">
              <div className="h-8 w-8 rounded-full bg-premium-dark-700 flex items-center justify-center mr-3">
                {community.icon}
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-medium">p/{community.name}</h3>
